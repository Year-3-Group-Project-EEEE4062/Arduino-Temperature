...
int depth_check = 1;      // 1: max_depth, 2: halfway_depth, 3: surface_depth 0: finish surface depth
...
float threshold = 6;         // threshold for maximum and surface depth

// Bool from RPi
bool boatBusy = 1;          // 1: take temperature, 0: quit temperature
...
void checkSurfaceDepth(int checkTemp){
...
void arrangeData(){
  ...
  for(int j = 0; j<6; j++){
    Serial.println(transmit_data_arr[j]);
  }
}
...
void initTemperature(){
  if(temp_check == 1){
    ...
    switch (depth_check) {
    case 1:
    // Serial.println("MAXMAXMAX");
      checkMaxDepth();
      break;
    case 2:
    // Serial.println("HALFHALFHALF");
      checkHalfDepth();
      break;
    case 3:
    // Serial.println("SURFACESURFACESURFACE");
      checkSurfaceDepth(1);
      break;
    }
  }
